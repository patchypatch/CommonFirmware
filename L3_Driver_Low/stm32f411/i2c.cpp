...
void i2c_enable_clock(uint8_t i2c_channel) {
	/* Enable I2C1 clock */
...
void i2c_write8(uint8_t i2c_channel, uint8_t sid, uint8_t addr, uint8_t data) {
	...
	while( I2C1->SR2 & (1 << I2C_SR2_BUSY_Msk) ) { printf("wait1"); }
	...
	while( !(I2C1->SR1 & I2C_SR1_SB_Msk) ) { printf("wait2"); } // wait for hardware to set the SB (start bit)
	...
	while( !(I2C1->SR1 & I2C_SR1_ADDR_Msk) ) { printf("wait3"); } 
	...
	while( !(I2C1->SR1 & I2C_SR1_TXE_Msk) ) { printf("wait4"); }
	...
	while( !(I2C1->SR1 & I2C_SR1_TXE_Msk) ) { printf("wait5"); }
	...
	while( !(I2C1->SR1 & I2C_SR1_TXE_Msk) ) { printf("wait5"); }
	while( !(I2C1->SR1 & I2C_SR1_BTF_Msk) ) { printf("wait6"); }
...
/* Master reciever, fig. 165 in stm32f411xce reference manual */
uint16_t i2c_read16(uint8_t i2c_channel, uint8_t sid) {
	uint16_t data = 0;
	printf("read0");
	// Wait until BUSY bit of I2C_SR2 is cleared (bus is idle)
	while( I2C1->SR2 & (1 << I2C_SR2_BUSY_Msk) ) { printf("read1"); }

	// S: Write START bit of I2C_CR1 (generates start condition on bus)	
	I2C1->CR1 |= (1 << I2C_CR1_START_Pos); // makes this chip a master
	
	// EV5: Wait until START is complete (SB in I2C_SR1 is set)
	while( !(I2C1->SR1 & I2C_SR1_SB_Msk) ) { printf("read2"); } // wait for hardware to set the SB (start bit)

	// Address: Write the slave address (bits 1-7) and the R/W bit as read (bit 0 to 1)
	uint8_t temp = (sid << 1) + 1;
	printf("0x%02x\n", temp);
	I2C1->DR = (sid << 1) + 1; // +1 after shift puts a 1 in bit 0

	// A: Wait until ADDR bit of I2C_SR1 register is set (slave address sent)
	while( !(I2C1->SR1 & I2C_SR1_ADDR_Msk) ) { printf("read3"); } 
	...
	while( !(I2C1->SR1 & I2C_SR1_BTF_Msk) ) { printf("read4"); }
